...
def apriori(itemsets, threshold):
    ...
    while True:
        ...
        if not new_candidates:
            break
        # Compute support for new candidates
        support_new = {itemset: support_count(itemset, itemsets) for itemset in new_candidates}
        # Update the support dictionary with new values
        support.update(support_new)
        # Filter by the support threshold to find current frequent itemsets
    ...
    return [(set(itemset), round(support[itemset], 2)) for itemset in all_frequent]
